...
class MinimaxAgent(MultiAgentSearchAgent):
  ...
  def getAction(self, gameState):
    ...
    return minimax(gameState, 0, 1)[1] #start at pacman id and depth 0
...
class AlphaBetaAgent(MultiAgentSearchAgent):
  ...
  def getAction(self, gameState):
    ...
    def alphabeta(gameState, agentIndex, currentDepth, alpha, beta):
      ...
      if currentDepth > self.depth or not legalMoves:
      ...
      for m in legalMoves:
        ...
        if agentIndex == 0:
          alpha = max(alpha, alphabeta(successorGameState, nextAgentIndex, nextDepth, alpha, beta)[0])
          if beta <= alpha:
            break
          return alpha, m
        else:
          beta  = min(beta,  alphabeta(successorGameState, nextAgentIndex, nextDepth, alpha, beta)[0])
          if beta <= alpha:
            break
          return beta, m
    #####
    ...
    ret, m = alphabeta(gameState, 0, 1, -999999, 999999)

    # print ret, m

    return m

    # firstMoves = gameState.getLegalActions(0)

    # maxVal = -999999
    # maxI = 0

    # for i, m in enumerate(firstMoves):
    #   newVal = alphabeta(gameState.generateSuccessor(0, m), 1, 1, -999999, 999999)
    #   if newVal > maxVal:
    #     maxVal = newVal
    #     maxI = i

    # print firstMoves[maxI], maxVal

    # return firstMoves[maxI]
